...
Split: 60% train / 20% validation / 20% test (true holdout)
...
# ── KNN spatial feature ──────────────────────────────────────────────
# Reads postal_coords.csv (run nmrk_geocode_postal.py once to generate it)
# Adds knn_median_psf: median PSF of the K geographically nearest
# transactions in the prior 12 months — a precise micro-neighbourhood
# price signal that outperforms Planning Area for high-CV zones.
KNN_K         = 10    # number of nearest transactions
KNN_MONTHS    = 12    # lookback window for neighbour search
COORDS_CSV    = OUTPUT_DIR / "postal_coords.csv"

def haversine_vec(lat1, lon1, lats, lons):
    """Vectorised haversine distance (km) from one point to arrays."""
    R = 6371.0
...
KNN_FEATURE_AVAILABLE = False
if COORDS_CSV.exists():
    coords_df = pd.read_csv(COORDS_CSV, dtype={"Postal Code": str})
    coords_df["Postal Code"] = coords_df["Postal Code"].str.zfill(6)
    coords_df = coords_df.dropna(subset=["Lat", "Lon"])

    # Merge lat/lon into full dataset
    df_full["Postal Code"] = df_raw["Postal Code"].astype(str).str.zfill(6).reindex(df_full.index)
    df_full = df_full.merge(coords_df[["Postal Code","Lat","Lon"]], on="Postal Code", how="left")
    df_full.drop(columns=["Postal Code"], inplace=True)

    # Compute knn_median_psf for each row using prior 12-month transactions
    TARGET_COL = "Unit Price ($ PSF)"
    df_sorted  = df_full.sort_values("Sale Date").reset_index(drop=True)
    lookback   = pd.DateOffset(months=KNN_MONTHS)
    knn_vals   = np.full(len(df_sorted), np.nan)

    lat_arr = df_sorted["Lat"].values
    lon_arr = df_sorted["Lon"].values
    psf_arr = df_sorted[TARGET_COL].values
    date_arr= df_sorted["Sale Date"].values

    print(f"  Computing KNN spatial feature (k={KNN_K}, {KNN_MONTHS}M lookback) …")
    for i in range(len(df_sorted)):
        if np.isnan(lat_arr[i]) or np.isnan(lon_arr[i]):
            continue
        cutoff_dt = date_arr[i] - np.timedelta64(KNN_MONTHS * 30, 'D')
        # Candidate mask: prior transactions within lookback window
        mask = (date_arr < date_arr[i]) & (date_arr >= cutoff_dt) & (~np.isnan(lat_arr))
        if mask.sum() < 3:
            continue
        dists = haversine_vec(lat_arr[i], lon_arr[i], lat_arr[mask], lon_arr[mask])
        k     = min(KNN_K, len(dists))
        top_k = np.argsort(dists)[:k]
        knn_vals[i] = np.median(psf_arr[mask][top_k])

    df_sorted["knn_median_psf"] = knn_vals
    # Fill NaN (no neighbours found) with planning area median
    area_med = df_sorted.groupby("Planning Area")[TARGET_COL].transform("median")
    df_sorted["knn_median_psf"] = df_sorted["knn_median_psf"].fillna(area_med)
    df_full = df_sorted  # replace with enriched version
    KNN_FEATURE_AVAILABLE = True
    pct_filled = (~np.isnan(knn_vals)).mean() * 100
    print(f"  KNN feature computed: {pct_filled:.1f}% of rows have neighbour data")
else:
    print("  postal_coords.csv not found — run nmrk_geocode_postal.py first")
    print("  Proceeding without KNN spatial feature.")
...
TARGET = "Unit Price ($ PSF)"
# Add KNN spatial feature if postal_coords.csv was available
KNN_COLS = ["knn_median_psf"] if KNN_FEATURE_AVAILABLE else []
EXTRA_FEATURES = EXTRA_LOC_FEATURES + KNN_COLS  # location + spatial features
...
for sale_key, sale_types in SALE_SEGMENTS.items():
    ...
    for period, cutoff in cutoffs.items():
        ...
        # ── 60 / 20 / 20 split (done before encoding to prevent leakage) ───────
        # We split first so target encoding is fitted on train set only
        from sklearn.model_selection import train_test_split as _tts
        df_train_raw, df_temp_raw = _tts(df, test_size=0.40, random_state=42)
        df_val_raw,   df_test_raw = _tts(df_temp_raw, test_size=0.50, random_state=42)

        # ── Target encode Project Name ────────────────────────────────────
        # Use last 3 years of full segment data so recent price levels
        # dominate (e.g. Ardmore Park at S$4,000+ psf in 2026, not the
        # historical average which includes older S$2,500 transactions).
        # Rare projects still covered because 3 years >> 6-month window.
        te_cutoff_3y = df_seg["Sale Date"].max() - pd.DateOffset(years=3)
        df_seg_recent = df_seg[df_seg["Sale Date"] >= te_cutoff_3y]
        global_median = float(df[TARGET].median())  # period median as fallback
        # ── DEBUG: print Ardmore Park values to confirm 3Y window ─────────
        _ap = df_seg_recent[df_seg_recent["Project Name"].str.upper() == "ARDMORE PARK"]
        print(f"  [DEBUG] df_seg max date: {df_seg['Sale Date'].max().strftime('%b %Y')}  |  3Y cutoff: {te_cutoff_3y.strftime('%b %Y')}")
        print(f"  [DEBUG] Ardmore Park rows in 3Y window: {len(_ap)}  |  median PSF: S${_ap[TARGET].median():.0f}" if len(_ap) > 0 else f"  [DEBUG] Ardmore Park: 0 rows in 3Y window — will use all-time fallback")
        project_te_map = (
            df_seg_recent.groupby("Project Name")[TARGET]
            .median()
            .to_dict()
        )
        # For projects with no transactions in last 3 years, fall back to all-time median
        project_te_all = (
            df_seg.groupby("Project Name")[TARGET]
            .median()
            .to_dict()
        )
        for proj, val in project_te_all.items():
            if proj not in project_te_map:
                project_te_map[proj] = val
